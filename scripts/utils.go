...
func Usage(){
	...
	fmt.Fprintf(os.Stdout,"\t%s %s.\n", YellowBOLDFg("-timeout"), MagBoldFg("int"))
	fmt.Fprintf(os.Stdout,"\t\t HTTP Connection timeout. Default: 10s\n")
	fmt.Fprintf(os.Stdout,"\t%s %s.\n", YellowBOLDFg("-auth-type"), MagBoldFg("string"))
	fmt.Fprintf(os.Stdout,"\t\t Authentication type: basic or digest. Default: basic\n")
	fmt.Fprintf(os.Stdout,"\t%s %s.\n", YellowBOLDFg("-threads"), MagBoldFg("int"))
	fmt.Fprintf(os.Stdout,"\t\t Number of concurrent threads (goroutines). Default: 50\n")
	os.Exit(0)
}
